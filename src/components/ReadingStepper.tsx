      ...
      {fields.map(({ key, labelKey, unitKey, color }) => (
        <div key={key} className="flex items-center gap-1.5 sm:gap-3">
          ...
          <button
            ...
            className="w-10 h-10 sm:w-12 sm:h-12 rounded-full bg-gray-100 dark:bg-gray-700 text-2xl font-bold text-gray-600 dark:text-gray-200
          ...
          <input
            ...
            className={`flex-1 min-w-0 text-center text-3xl sm:text-4xl font-bold border-2 border-gray-200 dark:border-gray-600 dark:bg-gray-900 rounded-xl
                         py-2 focus:ring-2 focus:ring-primary-500 focus:border-primary-500 ${color ?? ""}`}
          />
          <button
            type="button"
            onClick={() => onChange(key, values[key] + 1)}
